...
class AsciiArt:
    ...
    def _img_to_art(
        ...
        if mode == Modes.TERMINAL:
            ...
            for line in lines:
                ...
                for character in line:
                    current_color = (
                        self.get_charcode(front)
                        if front
                        else character["terminal-color"]
                    )
        ...
        if mode == Modes.HTML_TERMINAL:
            ...
            for line in lines:
                ...
                for character in line:
                    art += (
                        f'<span style="color:{character["terminal-hex-color"]}">'
                        + character["character"]
                        + "</span>"
                    )
        ...
        if mode == Modes.HTML_FULL_COLOR:
            ...
            for line in lines:
                ...
                for character in line:
                    art += (
                        f'<span style="color:{character["full-hex-color"]}">'
                        + character["character"]
                        + "</span>"
                    )
    ...
    def color_to_hex(color: Union[Front, Back, str]) -> str:
        if isinstance(color, (Front, Back)):
            ...
            if color.name == "WHITE":
                return "#FFFFFF"
            if color.name == "LIGHTBLACK":
                return "#222222"
            if color.name == "LIGHTRED":
                return "#FF6666"
            if color.name == "LIGHTGREEN":
                return "#66FF66"
            if color.name == "LIGHTYELLOW":
                return "#FFFF66"
            if color.name == "LIGHTBLUE":
                return "#6666FF"
            if color.name == "LIGHTMAGENTA":
                return "#FF66FF"
            if color.name == "LIGHTCYAN":
                return "#66FFFF"
            if color.name == "LIGHTWHITE":
                return "#FFFFFF"
    ...
    def get_color_data(char: str, rgb: Union[list, tuple], brightness: float) -> dict:
        ...
        return {
            ...
            "full-hex-color": "#{:02x}{:02x}{:02x}".format(
                *(int(c * 200 + 55) for c in rgb)
            ),
        }
        ...
        if isinstance(result, list):  # win32 file list
            try:
                ...
                if isinstance(first, os.PathLike):
                    return Image.open(os.fspath(first))
    ...
    def _load_clipboard_linux(cls) -> Image.Image:
        try:
            ...
            from gi.repository import Gtk, Gdk  # type: ignore[import]
    ...
    def _load_swarmui(
        ...
        if not server:
            raise ValueError(
                "You must set up a SwarmUI server before accessing SwarmUI"
            )
        ...
        if model == "auto":
            ...
            if debug:
                with open(
                    str(int(time())) + "_swarmui_ListModels_response.txt", "w"
                ) as f:
                    f.write(str(models))
        ...
        if debug:
            with open(
                str(int(time())) + "_swarmui_GenerateText2Image_response.txt", "w"
            ) as f:
